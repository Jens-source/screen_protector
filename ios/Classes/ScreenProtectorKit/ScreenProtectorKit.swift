...
public class ScreenProtectorKit {
    ...
    private var screenshotHost: UIView?
    ...
    public func configurePreventionScreenshot() {
        if Thread.isMainThread {
            configurePreventionScreenshotOnMain()
        } else {
            DispatchQueue.main.async { [weak self] in
                self?.configurePreventionScreenshotOnMain()
            }
        }
    }
    
    private func configurePreventionScreenshotOnMain() {
        guard let w = window else { return }
        
        // Already installed.
        if screenPrevent.superview != nil { return }
        
        // Wait until the window is key/visible to avoid illegal window constraints.
        ...
        let host = UIView(frame: w.bounds)
        host.isHidden = true
        host.translatesAutoresizingMaskIntoConstraints = false
        w.addSubview(host)
        NSLayoutConstraint.activate([
            host.leadingAnchor.constraint(equalTo: w.leadingAnchor),
            host.trailingAnchor.constraint(equalTo: w.trailingAnchor),
            host.topAnchor.constraint(equalTo: w.topAnchor),
            host.bottomAnchor.constraint(equalTo: w.bottomAnchor),
        ])
        
        screenPrevent.translatesAutoresizingMaskIntoConstraints = false
        screenPrevent.alpha = 0.01
        screenPrevent.isHidden = false
        screenPrevent.isUserInteractionEnabled = false
        host.addSubview(screenPrevent)
        NSLayoutConstraint.activate([
            screenPrevent.centerXAnchor.constraint(equalTo: host.centerXAnchor),
            screenPrevent.centerYAnchor.constraint(equalTo: host.centerYAnchor),
        ])
